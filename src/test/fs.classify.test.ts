...
import {classify_fs_error, type FsError} from '$lib/fs.ts';
...
describe('classify_fs_error', () => {
	test('classifies ENOENT as not_found', () => {
		const err = Object.assign(new Error('no such file'), {code: 'ENOENT'});
		const result = classify_fs_error(err);
	...
	test('plain object with code property is classified', () => {
		const result = classify_fs_error({code: 'ENOENT', message: 'not a real Error'});
		assert.strictEqual(result.kind, 'not_found');
	...
	test('result is a discriminated union exhaustively narrowable by kind', () => {
		...
		assert.strictEqual(label(classify_fs_error(err)), 'missing');
	});
	...
	test('classifies real ENOENT thrown by readFile', async () => {
		try {
			...
			const result = classify_fs_error(error);
			assert.strictEqual(result.kind, 'not_found');
		}
	});
